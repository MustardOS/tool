...
REPO_URL="https://github.com/mupen64plus/mupen64plus-core.git"
REPO_DIR="$SCRIPT_DIR/mupen64plus-core"
PATCH_DIR="$SCRIPT_DIR/../mp64-patches"
REPO_NAME="$(basename "$REPO_DIR")"

echo "[1/8] Cloning mupen64plus-core..."
rm -rf "$REPO_DIR"
...
echo "[4/8] Configuring Building mupen64plus-core"
...
VULKAN=0 USE_GLES=1 NEW_DYNAREC=1 OPTFLAGS="-O3" V=1 PIE=1 ACCURATE_FPU=1 HOST_CPU=aarch64 GL_CFLAGS="" GL_LDLIBS="-lEGL -lGLESv2" PKG_CONFIG="aarch64-buildroot-linux-gnu-pkg-config" \
make -C "mupen64plus-core/projects/unix" clean

VULKAN=0 USE_GLES=1 NEW_DYNAREC=1 OPTFLAGS="-O3" V=1 PIE=1 ACCURATE_FPU=1 HOST_CPU=aarch64 GL_CFLAGS="" GL_LDLIBS="-lEGL -lGLESv2" PKG_CONFIG="aarch64-buildroot-linux-gnu-pkg-config" \
make -C "mupen64plus-core/projects/unix" -j"$(nproc)" all

echo "[6/8] Stripping mupen64plus-core binary..."
$STRIP "mupen64plus-core/projects/unix/libmupen64plus.so.2.0.0" || true
...
cp -Pv "mupen64plus-core/projects/unix/libmupen64plus.so.2" "$SCRIPT_DIR/mupen64plussa/$MP_BIN/"
...
cp -v "mupen64plus-core/data/mupencheat.txt" "$SCRIPT_DIR/mupen64plussa/$MP_BIN/."
