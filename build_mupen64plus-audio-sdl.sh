...
REPO_URL="https://github.com/mupen64plus/mupen64plus-audio-sdl.git"
REPO_DIR="$SCRIPT_DIR/mupen64plus-audio-sdl"
PATCH_DIR="$SCRIPT_DIR/mp64-patches"
REPO_NAME="$(basename "$REPO_DIR")"

echo "[1/8] Cloning mupen64plus-audio-sdl..."
rm -rf "$REPO_DIR"
...
echo "[4/8] Configuring Building mupen64plus-audio-sdl"
...
VULKAN=0 USE_GLES=1 NEW_DYNAREC=1 OPTFLAGS="-O3" V=1 PIE=1 ACCURATE_FPU=1 HOST_CPU=aarch64 GL_CFLAGS="" GL_LDLIBS="-lEGL -lGLESv2" PKG_CONFIG="aarch64-buildroot-linux-gnu-pkg-config" SDL_CONFIG="aarch64-buildroot-linux-gnu-sdl2-config" \
make -C "mupen64plus-audio-sdl/projects/unix" -j"$(nproc)" all

echo "[6/8] Stripping mupen64plus-audio-sdl binary..."
$STRIP "mupen64plus-audio-sdl/projects/unix/mupen64plus-audio-sdl.so" || true

echo "[7/8] Move Binary to mupen64plussa"
mkdir -p "$SCRIPT_DIR/mupen64plussa/$MP_BIN"
cp -v "mupen64plus-audio-sdl/projects/unix/mupen64plus-audio-sdl.so" "$SCRIPT_DIR/mupen64plussa/$MP_BIN/plugins/."
